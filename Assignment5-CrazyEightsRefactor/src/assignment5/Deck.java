...
public class Deck {
     ...
     * @param numOfPlayers sets how many decks will be 
     * created for the match. The max number of players is 10
     */
    public Deck(int numOfPlayers, boolean isDiscard) {
    	int loopCt = 0, cardCt = 0;
    	if(numOfPlayers < 4) {
    		deck = new Card[52]; // Deck for up to 3 players
    		loopCt = 1;
    	} else if(numOfPlayers < 8) {
    		deck = new Card[104]; // Deck for up to 7 players
    		loopCt = 2;
    	} else {
    		deck = new Card[156]; // Deck for up to 10 players
    		loopCt = 3;
    	}
    	if(!isDiscard) {
    ...
    /**
     * Add cards ONLY TO Discard Deck
