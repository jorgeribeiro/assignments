...
public class RedBlackTree<T extends Comparable<T>> extends BinarySearchTree<T> {
    ...
    protected Node<T> addValue(T id) {
        ...
        while (node != null) {
            if (node.id == null) {
                ...
                break;
            } else if (genericComparator.compare(id, node.id) <= 0) {
                node = node.lesser;
            } else {
                node = node.greater;
            }
    ...
    protected boolean validateNode(Node<T> node) {
        ...
        if (!lesser.isLeaf()) {
            // Check BST property
        	boolean lesserCheck = genericComparator.compare(lesser.id, rbNode.id) <= 0;
        ...
        if (!greater.isLeaf()) {
            // Check BST property
            boolean greaterCheck = genericComparator.compare(greater.id, rbNode.id) > 0;
            if (!greaterCheck) 
                return false;
            // Check red-black property
