...
public class BTree<T extends Comparable<T>> implements ITree<T> {
     ...
     */
    private Comparator<T> genericComparator = new Comparator<T>() {
		@Override
    ...
    public boolean add(T value) {
        if (root == null) {
            ...
            while (node != null) {
                ...
                if (genericComparator.compare(value, lesser) <= 0) {
                ...
                if (genericComparator.compare(value, greater) > 0) {
                ...
                for (int i = 1; i < node.numberOfKeys(); i++) {
                    ...
                    if (genericComparator.compare(value, prev) > 0 && genericComparator.compare(value, next) <= 0) {
    ...
    private Node<T> getNode(T value) {
        ...
        while (node != null) {
            ...
            for (int i = 0; i < numberOfKeys; i++) {
                ...
                if (genericComparator.compare(currentValue, value) == 0) {
                    return node;
                }
                ...
                if (next <= last) {
                    ...
                    if (genericComparator.compare(currentValue, value) < 0 && genericComparator.compare(nextValue, value) > 0) {
    ...
    private int getIndexOfPreviousValue(Node<T> node, T value) {
        for (int i = 1; i < node.numberOfKeys(); i++) {
            T t = node.getKey(i);
            if (genericComparator.compare(t, value) >= 0)
                return i - 1;
        }
    ...
    private boolean validateNode(Node<T> node) {
        ...
        if (keySize > 1) {
            ...
            for (int i = 1; i < keySize; i++) {
                T p = node.getKey(i - 1);
                T n = node.getKey(i);
                if (genericComparator.compare(p, n) > 0)
                    return false;
            }
        }
        ...
        if (genericComparator.compare(first.getKey(first.numberOfKeys() - 1), node.getKey(0)) > 0)
            return false;

        Node<T> last = node.getChild(node.numberOfChildren() - 1);
        // The last child's first key should be greater than the node's last key
        if (genericComparator.compare(last.getKey(0), node.getKey(node.numberOfKeys() - 1)) < 0)
            return false;
        ...
        for (int i = 1; i < node.numberOfKeys(); i++) {
            ...
            if (genericComparator.compare(p, c.getKey(0)) > 0)
                return false;
            if (genericComparator.compare(n, c.getKey(c.numberOfKeys() - 1)) < 0)
                return false;
        }
    ...
    private static class Node<T extends Comparable<T>> {
         ...
         * @author Jorge Ribeiro
         */
        private Comparator<Node<T>> nodeComparator = new Comparator<Node<T>>() {
            @Override
            public int compare(Node<T> arg0, Node<T> arg1) {
                String s0 = arg0.getKey(0).toString();
            	String s1 = arg1.getKey(0).toString();
        ...
        private int indexOf(T value) {
            for (int i = 0; i < keysSize; i++) {
            	if (genericComparator.compare(keys[i], value) == 0) return i;
            }
            return -1;
        }
        ...
        private void addKey(T value) {
            keys[keysSize++] = value;
            Arrays.sort(keys, 0, keysSize, genericComparator);
        }
        ...
        private T removeKey(T value) {
            ...
            for (int i = 0; i < keysSize; i++) {
            	if (genericComparator.compare(keys[i], value) == 0) {
                    found = true;
                    removed = keys[i];
                } else if (found) {
        ...
        private boolean addChild(Node<T> child) {
            ...
            Arrays.sort(children, 0, childrenSize, nodeComparator);
            return true;
        }
