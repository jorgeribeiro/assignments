...
public class BinarySearchTree<T extends Comparable<T>> implements ITree<T> {
    ...
    protected Comparator<T> genericComparator = new Comparator<T>() {
		@Override
    ...
    protected Node<T> addValue(T value) {
        ...
        Node<T> node = root;
        while (node != null) {
            if (genericComparator.compare(newNode.id, node.id) <= 0) {
    ...
    protected Node<T> getNode(T value) {
        ...
        while (node != null && node.id != null) {
        	if (genericComparator.compare(value, node.id) < 0) {
                node = node.lesser;
            } else if (genericComparator.compare(value, node.id) > 0) {
                node = node.greater;
            } else if (genericComparator.compare(value, node.id) == 0) {
                return node;
            }
    ...
    protected void replaceNodeWithNode(Node<T> nodeToRemoved, Node<T> replacementNode) {
        ...
        if (parent == null) {
            ...
            if (root != null)
                root.parent = null;
        } else if (parent.lesser != null && (genericComparator.compare(parent.lesser.id, nodeToRemoved.id) == 0)) {
            parent.lesser = replacementNode;
            if (replacementNode != null)
                replacementNode.parent = parent;
        } else if (parent.greater != null && (genericComparator.compare(parent.greater.id, nodeToRemoved.id) == 0)) {
    ...
    protected boolean validateNode(Node<T> node) {
        ...
        if (lesser != null && lesser.id != null) {
            lesserCheck = (genericComparator.compare(lesser.id, node.id) <= 0);
            if (lesserCheck)
        ...
        if (greater != null && greater.id != null) {
            greaterCheck = (genericComparator.compare(greater.id, node.id) > 0);
            if (greaterCheck)
    ...
    public static <T extends Comparable<T>> T[] getDFS(DepthFirstSearchOrder order, Node<T> start, int size) {
        ...
        while (index < size && node != null) {
            ...
            if (order == DepthFirstSearchOrder.inOrder) {
                ...
                if (lesser != null && lesser.id != null) {
                    node = lesser;
                } else if (greater != null && greater.id != null) {
                    node = greater;
                } else if (added.contains(node)) {
                    node = parent;
                } else {
            ...
            } else if (order == DepthFirstSearchOrder.postOrder) {
                if (lesser != null && lesser.id != null) {
                    node = lesser;
                } else {
                    if (greater != null && greater.id != null) {
